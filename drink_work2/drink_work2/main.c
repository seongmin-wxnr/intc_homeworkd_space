...
const char list_A[ARRAY_SIZE_AX][ARINF_] = {
		"진저에일",
		"클럽소다",
		"파워에이드",
		"콜라",
...
const char list_B[ARRAY_SIZE_BX][ARINF_] = {
		"오랜지쥬스",
		"딸기쥬스",
		"조니워터",
		"콜라",
...
unsigned long  main_list() {
	printf_s("=== A마켓 판매 물품 리스트 ===\n\n");
	for (int i = 0; i < ARRAY_SIZE_AX; i++) {
		for (int z = 0; z < ARRAY_SIZE_Y; z++) {
			printf("%c", list_A[i][z]);
		}
		printf("\n");
	}
	printf_s("\n=== B마켓 판매 물품 리스트 ===\n\n");
	for (int i = 0; i < ARRAY_SIZE_BX; i++) {
		for (int z = 0; z < ARRAY_SIZE_Y; z++) {
			printf("%c", list_B[i][z]);
		}
		printf("\n");
...
char isOnlyCorresponed_B() {
	printf_s("=== B업체에서만 판매중인 음료 ===\n");
	for (int i = 0; i < ARRAY_SIZE_BX; i++) {
		int isCorresponed_v = 0;
		for (int z = 0; z < ARRAY_SIZE_AX; z++) {
